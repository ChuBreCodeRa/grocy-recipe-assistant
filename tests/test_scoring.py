...
def test_ai_suggest_recipes(monkeypatch):
    ...
    monkeypatch.setattr(recipes_mod, "get_inventory_ingredient_names", mock_get_inventory_ingredient_names)
    ...
    def mock_fetch_recipes_from_spoonacular(ingredients, number=10, max_ready_time=None, dietary_restrictions=None):
        # Updated to accept all parameters used in the real function
        return mock_base_recipes
    monkeypatch.setattr(recipes_mod, "fetch_recipes_from_spoonacular", mock_fetch_recipes_from_spoonacular)

    # Mock Spoonacular: fetch_recipe_details
    mock_details = {
        1: {"readyInMinutes": 25, "extendedIngredients": [{"name": "chicken"}, {"name": "rice"}, {"name": "onion"}, {"name": "soy sauce"}]},
        2: {"readyInMinutes": 20, "extendedIngredients": [{"name": "chicken"}, {"name": "rice"}]},
        3: {"readyInMinutes": 75, "extendedIngredients": [{"name": "chicken"}, {"name": "truffle oil"}, {"name": "caviar"}]}
    }
    ...
    mock_tastes = {
        1: {"sweetness": 20, "saltiness": 70, "sourness": 10, "bitterness": 5, "savoriness": 80, "fattiness": 60}, # Good match
        2: {"sweetness": 10, "saltiness": 50, "sourness": 5, "bitterness": 5, "savoriness": 60, "fattiness": 40}, # Okay match
        3: {"sweetness": 5, "saltiness": 20, "sourness": 15, "bitterness": 30, "savoriness": 40, "fattiness": 70}  # Poor match
    }
    def mock_fetch_recipe_taste_profile(recipe_id):
        return mock_tastes.get(recipe_id)
    monkeypatch.setattr(recipes_mod, "fetch_recipe_taste_profile", mock_fetch_recipe_taste_profile)

    # Mock AI: classify_ingredients_with_ai
    def mock_classify_ingredients(recipe, user_inventory, recipe_ingredients_list):
        return [{"ingredient": "chicken", "category": "Essential", "in_inventory": True, "confidence": 0.9}]
