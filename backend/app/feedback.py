    ...
    try:
        ...
        try:
            ...
            if not isinstance(result, dict) or \
               "taste_profile" not in result or \
               not isinstance(result["taste_profile"], dict) or \
               "effort_tag" not in result or \
               "sentiment" not in result:
            ...
            required_tastes = ["sweetness", "saltiness", "sourness", "bitterness", "savoriness", "fattiness"]
            if not all(k in result["taste_profile"] and isinstance(result["taste_profile"][k], (int, float)) 
...
def store_feedback(user_id, recipe_id, rating, review_text, effort_tag, sentiment, taste_profile):
    try:
        ...
        ''', (
            user_id, recipe_id, rating, review_text, effort_tag, sentiment,
            taste_profile.get("sweetness"), taste_profile.get("saltiness"), taste_profile.get("sourness"),
            taste_profile.get("bitterness"), taste_profile.get("savoriness"), taste_profile.get("fattiness")
        ))
        conn.commit()
        cur.close()
        conn.close()
